 ...
 */
public class LogPasseggeroController implements Initializable {
    ...
    @FXML
    void controllogin(ActionEvent event) throws SQLException {
        Connessione conn;
        conn = Connessione.getConnessione();
        String query  = "select * from PASSEGGERI WHERE UTENTE = ? and PASSWORD = ?";
            try (PreparedStatement ps = conn.connect.prepareStatement(query)) {
                ...
                while(rs.next())
                {
                    verifica_nome = rs.getString("UTENTE");
